...
export default function MapSideTab({ open, setClose }) {
  const navigate = useNavigate();

  return (
    <TabWrapper style={open ? {transform: 'translateX(100%)'} : {transform: 'translateX(-100%)'}}>
      ...
      <TabPaper >
        ...
        <UserInfo onClick={() => navigate('/myPage')}>
          <img alt={'profile'}/>
          <Grid
            mystyles={'width: auto; padding-left: 13px;'}
          >
            <UserText><span>Lv.77</span>강윤지</UserText>
            <Grid
              mystyles={
        ...
        <Grid
          mystyles={
            'height: calc(100% - 144px); overflow: scroll; '
          }
        >
          <QuestListConatiner
            title={"완료한 퀘스트"}
